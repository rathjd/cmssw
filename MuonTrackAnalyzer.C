...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH3F *ChargeVeracity=  new TH3F("ChargeVeracity","matched to same tp charge veracity;#eta(tp);p_{T}(tp);sign(c_{1}#times c_{2})",25,0.0,2.5,20,0.,100.,2,-1.5,1.5);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	Current.SetPtEtaPhiM(tp_pt->at(tp),tp_eta->at(tp),tp_phi->at(tp),0.106);
	...
	if(mu>=200){ //is barrel muon
	  mu-=200;
	  DeltaPhiWindow->Fill(tpEta,tppT,fabs(TVector2::Phi_mpi_pi(matchtrk_phi->at(tp)-BMTF_muon_phi->at(mu))));
	  DeltaEtaWindow->Fill(tpEta,tppT,fabs(matchtrk_eta->at(tp)-BMTF_muon_eta->at(mu)));
	  DeltaPtWindow-> Fill(tpEta,tppT,fabs(matchtrk_pt->at(tp)-BMTF_muon_pt->at(mu)));
	  std::cout<<BMTF_muon_pt->at(mu)<<std::endl;
	}
	else if(mu>=100){ //is overlap muon
	  mu-=100;
	  DeltaPhiWindow->Fill(tpEta,tppT,fabs(TVector2::Phi_mpi_pi(matchtrk_phi->at(tp)-OMTF_muon_phi->at(mu))));
	  DeltaEtaWindow->Fill(tpEta,tppT,fabs(matchtrk_eta->at(tp)-OMTF_muon_eta->at(mu)));
	  DeltaPtWindow-> Fill(tpEta,tppT,fabs(matchtrk_pt->at(tp)-OMTF_muon_pt->at(mu)));
	  std::cout<<OMTF_muon_pt->at(mu)<<std::endl;
	}
	else{ //is endcap muon
	  DeltaPhiWindow->Fill(tpEta,tppT,fabs(TVector2::Phi_mpi_pi(matchtrk_phi->at(tp)-EMTF_muon_phi->at(mu))));
	  DeltaEtaWindow->Fill(tpEta,tppT,fabs(matchtrk_eta->at(tp)-EMTF_muon_eta->at(mu)));
	  DeltaPtWindow-> Fill(tpEta,tppT,fabs(matchtrk_pt->at(tp)-EMTF_muon_pt->at(mu)));	  
	  std::cout<<EMTF_muon_pt->at(mu)<<std::endl;
	}
      }
   }

   TFile *savefile=new TFile("test_savedHists.root","RECREATE");
