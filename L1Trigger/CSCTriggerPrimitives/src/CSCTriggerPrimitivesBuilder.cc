      ...
      for (int sect = min_sector; sect <= max_sector; sect++) {
        for (int subs = min_subsector; subs <= numsubs; subs++) {
          for (int cham = min_chamber; cham <= max_chamber; cham++) {
            ...
            const bool upgradeME11 = upgrade and stat == 1 and runME11Up_ and !runME11ILT_;
            const bool upgradeME21 = upgrade and stat == 2 and runME21Up_ and !runME21ILT_;
            const bool upgradeME31 = upgrade and stat == 3 and runME31Up_;
            const bool upgradeME41 = upgrade and stat == 4 and runME41Up_;
            ...
            // Phase-2 OTMBs without GEMs
            // ME3/1 and ME4/1, but also ME1/1 and ME2/1 if GEMs are not used
            else if (upgradeME11 or upgradeME21 or upgradeME31 or upgradeME41)
              tmb_[endc - 1][stat - 1][sect - 1][subs - 1][cham - 1] =
                  std::make_unique<CSCUpgradeMotherboard>(endc, stat, sect, subs, cham, conf);
            // default case: regular TMBs (ME1/2, ME1/3, ME2/2, ME3/2, ME4/2)
            else
              tmb_[endc - 1][stat - 1][sect - 1][subs - 1][cham - 1] =
        ...
        for (int subs = min_subsector; subs <= numsubs; subs++) {
          for (int cham = min_chamber; cham <= max_chamber; cham++) {
            ...
            if (upgradeGE11 or upgradeGE21) {
              ...
              // const std::vector<GEMCoPadDigi>& copads = tmbGEM->coPadProcessor->readoutCoPads();
